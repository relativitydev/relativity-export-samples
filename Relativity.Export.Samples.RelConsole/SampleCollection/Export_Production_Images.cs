...
public partial class BaseExportService
{
	...
	public async Task Export_FromProduction_Images()
	{
		...
		string? correlationID = $"Sample-Job-{nameof(Export_FromProduction_Images)}";
		...
		var artifactSettings = ExportArtifactSettingsBuilder.Create()
			...
			.ExportImages(settings => settings
				.WithImagePrecedenceArtifactIDs(new List<int> { productionID }) // Exports images
				.WithTypeOfImage(ImageType.SinglePage))
